  ...
  class Runner
    ...
    def run(path, color = nil, platform = nil)
      ...
      if config_files.count > 0
        ...
        if config.data.key?("skip_directories")
          skip_directories = config.data["skip_directories"]
        end

        if config.data.key?("only_directories")
          only_directories = config.data["only_directories"]
        end
      end
      ...
      if screenshots.count > 0
        ...
        screenshots.each do |full_path|
          next if skip_path?(full_path, skip_directories, only_directories)
          queue << full_path
        end
    ...
    def skip_path?(path, skip_directories, only_directories)
      ...
      return true if only_directories.any? && only_directories.none? { |dir| path.include?(dir) }
